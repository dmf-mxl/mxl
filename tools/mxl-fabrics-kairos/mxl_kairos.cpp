...
#include "mxl_kairos.hpp"

/*
    Example how to use:

        1- Start a target: ./mxl-fabrics-kairos -d <tmpfs folder> -f <NMOS JSON File> --service 1234
        2- Paste the target info that gets printed in stdout to the --target-info argument of the initiator.
        3- Start a sender: ./mxl-fabrics-kairos -i -d <tmpfs folder> -f <test source flow uuid> --service 1234 --target-info <targetInfo>
...
mxl_KAIROS::mxl_KAIROS(KairosOptions options)
    : _options{std::move(options)}
{}

void mxl_KAIROS::requestExit()
{
    g_exit_requested = 1;
}
...
int mxl_KAIROS::run()
{
    auto const& domain = _options.domain;
    auto const& flowConf = _options.flow;
    auto const& flowOptionsFile = _options.flowOptionsFile;
    auto const& node = _options.node;
    auto const& service = _options.service;
    auto const& provider = _options.provider;
    auto const& targetInfo = _options.targetInfo;
    auto const runAsInitiator = _options.runAsInitiator;
