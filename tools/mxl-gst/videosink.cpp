...
public:
    ...
    void pushVideoSample(uint8_t* payload, size_t payloadLen, std::uint64_t mxlTimestamp, std::uint64_t currentMxlTime)
    {
        ...
        GST_BUFFER_PTS(gstBuffer) = mxlTimestamp - _baseTimestamp + _highestLatency + 500000000;
    ...
    void pushAudioSamples(WrappedMultiBufferSlice const& payload, std::uint64_t firstSampleMxlTimestamp, std::uint64_t lastSampleMxlTimestamp,
        std::uint64_t currentMxlTime)
    {
        ...
        GST_BUFFER_PTS(gstBuffer) = firstSampleMxlTimestamp - _baseTimestamp + _highestLatency + 500000000;
...
private:
    ...
    /// Indicates the latency at which we are getting the data. We adjust PTS values based on this information to ensure fluent playback.
    std::uint64_t _highestLatency{0};
    ...
    {
        auto const latency = currentMxlTime > mxlTimestamp ? currentMxlTime - mxlTimestamp : 0;
        if (latency > _highestLatency)
        {
            _highestLatency = latency;
            MXL_WARN("Latency increase detected: {} ns", latency);
        }
    }
};
...
int real_main(int argc, char** argv, void*)
{
    ...
    std::optional<std::uint64_t> readTimeout;
    auto readTimeoutOpt = app.add_option("-t,--timeout", readTimeout, "The read timeout in ns, frame interval + 1 ms used if not specified");
    ...
    std::uint64_t readTimeoutNs;
    if (readTimeout)
    {
        readTimeoutNs = *readTimeout;
    }
    else
    {
        readTimeoutNs = static_cast<std::uint64_t>(1.0 * videoRate.denominator * (1'000'000'000.0 / videoRate.numerator));
        readTimeoutNs += 1'000'000ULL; // allow some margin.
    }
    ...
    while (!g_exit_requested)
    {
        ...
        uint8_t* payload;
        auto ret = mxlFlowReaderGetGrain(videoReader, grain_index, readTimeoutNs, &grain_info, &payload);
        if (ret != MXL_STATUS_OK)
        {
        ...
        if (audioReader)
        {
            ...
            WrappedMultiBufferSlice audioPayload;
