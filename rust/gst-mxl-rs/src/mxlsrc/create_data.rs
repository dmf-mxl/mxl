...
pub(crate) fn create_data(
    ...
    let head = flow_head_index(&data_state.grain_reader)?;
    let grain_count = discrete_grain_count(&data_state.grain_reader)?;
    ...
    if !data_state.is_initialized {
        ...
        data_state.index = head;
        data_state.is_initialized = true;
    }

    let (read_index, jumped) = match resolve_read_step(data_state.index, head, grain_count) {
    ...
    trace!("Getting data grain with index: {read_index}");
    ...
    let Some(pts) = pts_for_index(instance, read_index, &rate, subtrahend)? else {
        ...
        data_state.next_discont |= jumped || slot_discont;
    ...
    trace!(pts = ?buffer.pts(), index = read_index, "Produced data buffer");
    data_state.index = read_index + 1;
